...
std::vector<int> depthFirstSearch(const std::unordered_map<int, Node*> adjList, const int& root, const int& target) {
  ...
  while (!DFSStack.empty()) {
    Node* node = adjList.at(DFSStack.top());
    DFSStack.pop();
...
int main() {
  ...
  std::vector<int> dfsTraversal = depthFirstSearch(adjList, 1, 5);
